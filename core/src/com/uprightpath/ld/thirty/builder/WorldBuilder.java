...
public class WorldBuilder {
    ...
    public void save(WorldGroup worldGroup, String file) {

        for (World world : worldGroup.getWorlds()) {
            worlds.put(worlds.size, world);
            agents.put(agents.size, world.getPlayer());
            for (Agent agent : world.getAgents()) {
                if (!agents.containsValue(agent, true)) {
                    agents.put(agents.size, agent);
                }
            }
            for (Collision collision : world.getCollisions()) {
                if (!collisions.containsValue(collision, true)) {
                    collisions.put(collisions.size, collision);
                }
            }
            for (Platform platform : world.getPlatforms()) {
                if (!platforms.containsValue(platform, true)) {
                    platforms.put(platforms.size, platform);
                }
            }
            for (WorldInteractable collisionInteractable : world.getCollisionInteractables()) {
                if (!collisionInteractables.containsValue(collisionInteractable, true)) {
                    collisionInteractables.put(collisionInteractables.size, collisionInteractable);
                }
            }
            for (GlobalEventTrigger globalEventTrigger : world.getGlobalEventTriggers()) {
                if (!globalEventTriggers.containsValue(globalEventTrigger, true)) {
                    globalEventTriggers.put(globalEventTriggers.size, globalEventTrigger);
                }
            }
            for (WorldEventTrigger worldEventTrigger : world.getWorldEventTriggers()) {
                if (!worldEventTriggers.containsValue(worldEventTrigger, true)) {
                    worldEventTriggers.put(worldEventTriggers.size, worldEventTrigger);
                }
            }
        }
        for (Agent agent : agents.values()) {
            if (agent instanceof Interactable) {
                stories.put(stories.size, ((Interactable) agent).getStory());
            }
        }
        for (WorldInteractable collisionInteractable : collisionInteractables.values()) {
            stories.put(stories.size, collisionInteractable.getStory());
        }
        for (GlobalEventTrigger globalEventTrigger : globalEventTriggers.values()) {
    ...
    public static WorldGroup buildWorldGroup() {
        ...
        player = new PlayerAgent("Player", new Polygon(new float[]{0, 0, .7f, 0, .7f, 1.6f, 0, 1.6f}), new Polygon(new float[]{0, -.3f, .7f, -.3f, .7f, .3f, 0, .3f}));
        player.setRenderer(new PolygonRenderer(new Polygon(new float[]{0, 0, .7f, 0, .7f, 1.6f, 0, 1.6f}), Color.GREEN));
        player.setPosition(2.5f, 0f);
        player.setAgentController(new PlayerAgentController(player));
        player.setMaxDelta(new Vector2(.2f, 1f));
        ...
        world = new World(worldGroup, player, "Man of Blue: Sewer");
        world.setWorldDelta(new Vector2(0f, -.04f));
        world.addRenderable(new RenderableAdapter(new PolygonRenderer(new Polygon(new float[]{0, 2, 1, 2, 1, 1, 2, 0, 4, 0, 5, 1, 5, 2, 15, 2, 15, 10, 0, 10}), Color.BLUE)));
